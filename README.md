...
Также обнаружилась высокая корреляция `smart_198_raw` с `smart_197_raw`, аналогично `smart_240_raw` с `smart_9_raw`, `smart_241_raw` и `smart_242_raw`. Соответственно, был рассмотрен вариант удаления одного признака из группы коррелированных.
...
Все данные делились на 3 части: тренировочную, валидационную и тестовую. Тренировочная выборка затем делилась на обучающую и проверочную. Значения метрики *AUC ROC* обученных на обучающей выборке моделей для предсказаний валидационной выборки представлены в таблице 2.

Таблица 2 – Валидационное значение метрики *AUC ROC* различных моделей
Название | *AUC ROC*
--- | ---
XGBClassifier | 0,99295
RandomForest | 0,99422
IsolationForest | 0,96488
BalancedRandomForest | 0,99397
Blending | 0,99285

Исходя из рассмотрения таблицы 2, в качестве лучшей модели выбран случайный лес. Модель обучалась на всей тренировочной выборке с проверкой качества на валидационной. Значение метрики при классификации тестовой выборки составляет 0,99286. Значение точности (Precision), полноты (Recall) и меры F1 для каждого класса приведены в таблице 3.

Таблица 3 – Значения различных показателей качества классификации для каждого класса
Класс | Точность | Полнота | F1 мера | Число экземпляров
--- | --- | --- | --- | --- |
Работает | 1,00 | 1,00 | 1,00 | 66588 |
Вышел из строя | 0,75 | 0,93 | 0,83 | 1061 |

## Заключение

Были рассмотрены различные подходы к определению выхода из строя жестких дисков. Наилучший результат показала модель случайного леса. Полученные результаты дают решение поставленной задачи.
